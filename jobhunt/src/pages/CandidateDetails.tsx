...
const [isMARUploading, setIsMARUploading] = useState(false);
  ...
  /*
  |--------------------------------------------------------------------------
  | PROGRAM ACTIVITY STATES
  |--------------------------------------------------------------------------
  */

  const [programActivities, setProgramActivities] =
    useState<ProgramActivity[]>([]);

  const [programActivityModalOpen, setProgramActivityModalOpen] =
    useState(false);

  const [programActivityDate, setProgramActivityDate] =
    useState(new Date().toISOString().split("T")[0]);

  const [programActivityType, setProgramActivityType] =
    useState("Candidate Meeting");

  const [programActivityConductedBy, setProgramActivityConductedBy] =
    useState("");

  const [programActivitySubject, setProgramActivitySubject] =
    useState("");
    ...
    async function loadProgramActivities() {
      try {
        const response = await fetch(
          `${API_BASE_URL}/api/candidates/${candidateId}/program-activities`,
        );
        ...
        setProgramActivities(list);
      } catch (err) {
        console.error("Program activity loading error:", err);
      }
    }
...
async function saveDailyApplications() {
  ...
  */
  ...
  function openProgramActivityModal() {
    setProgramActivityDate(
      new Date().toISOString().split("T")[0],
    );
    setProgramActivityType("Candidate Meeting");
    setProgramActivityConductedBy(
      candidate?.owner || candidate?.assignedSpecialist || "",
    );
  ...
  async function saveProgramActivity() {
    if (!programActivityDate || !programActivityConductedBy.trim() || !programActivityNotes.trim()) {
      alert("Please enter the meeting date, conducted by, and meeting report.");
      return;
    }

    setSavingProgramActivity(true);
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(data?.message || "Failed to save program activity.");
      }

      const newActivity = data?.data;

      if (newActivity) {
        setProgramActivities((previous) => [newActivity, ...previous]);
      }

      setProgramActivityModalOpen(false);
      alert("Program activity saved successfully.");
    } catch (error) {
      console.error("Program activity save error:", error);
      alert(
        error instanceof Error
          ? error.message
          : "Unable to save program activity.",
      );
    } finally {
      setSavingProgramActivity(false);
    }
  }

  async function deleteProgramActivity(activity: ProgramActivity) {
    const confirmed = window.confirm(
      "Are you sure you want to delete this program activity?",
    );
    ...
    try {
      const response = await fetch(
        `${API_BASE_URL}/api/candidates/${candidateId}/program-activities/${encodeURIComponent(activity.id)}`,
        { method: "DELETE" },
      );
      ...
      if (!response.ok) {
        throw new Error(data?.message || "Failed to delete program activity.");
      }

      setProgramActivities((previous) =>
        previous.filter((item) => item.id !== activity.id),
      );

      alert("Program activity deleted successfully.");
    } catch (error) {
      console.error("Program activity delete error:", error);
      alert(
        error instanceof Error
          ? error.message
          : "Unable to delete program activity.",
      );
    }
  }
...
async function handleMARUpload() {
  ...
  */
  ...
  async function deleteMARReport(
    ...
    try {
      const response = await fetch(
        `${API_BASE_URL}/api/candidates/${candidateId}/mar/${encodeURIComponent(report.id)}`,
        {
          method: "DELETE",
        },
      );
      ...
      if (!response.ok) {
        throw new Error(
          data?.message ||
            `Unable to delete MAR report (${response.status})`,
        );
      }
      ...
      setMarPage((page) => {
        const remainingCount =
          uploadedMARReports.length - 1;
        const remainingPages = Math.max(
          1,
          Math.ceil(remainingCount / marPerPage),
        );
      ...
      if (
        previewMAR?.id === report.id
      ) {
        setPreviewMAR(null);
        setMarContent("");
      }

      alert("MAR report deleted successfully.");
    } catch (error) {
      console.error(
        "MAR report delete error:",
        error,
      );
  ...
  /*
  |--------------------------------------------------------------------------
  | DELETE NORMAL REPORT / INTERVIEW CALL
  |--------------------------------------------------------------------------
  */

  async function deleteUploadedReport(
    ...
    try {
      const response = await fetch(
        `${API_BASE_URL}/api/candidates/${candidateId}/reports/${encodeURIComponent(report.id)}`,
        {
          method: "DELETE",
        },
      );
      ...
      setReportsPage((page) => {
        ...
        const remainingPages = Math.max(
          1,
          Math.ceil(remainingCount / reportsPerPage),
        );
...
const totalProgramActivityPages = Math.ceil(
  programActivities.length / 8
);

const paginatedProgramActivities = programActivities.slice(
  0,
  8
);
...
>
  Applications
          ...
          <Button
            variant={
              activeTab === "program-activity"
                ? "default"
                : "outline"
            }
            size="sm"
            onClick={() =>
              setActiveTab("program-activity")
            }
          >
            Program Activity
          </Button>
                ...
                <td className="px-6 py-3 text-center">
                  <div className="flex items-center justify-center gap-1">
                    ...
                    <button
                      ...
                      onClick={() => deleteMARReport(report)}
                      title="Delete MAR report"
                      aria-label="Delete MAR report"
                    >
                ...
                <td className="px-6 py-3 text-center">
                  <div className="flex items-center justify-center gap-1">
                    ...
                    <button
                      type="button"
                      className="inline-flex size-8 items-center justify-center rounded-md text-destructive hover:bg-destructive/10"
                      onClick={() => deleteUploadedReport(report)}
                      title="Delete report"
          ...
          <Card className="mt-5">
            <CardHeader className="flex-row items-center justify-between">
              <div>
                ...
                <p className="mt-1 text-sm text-muted-foreground">
                  Store meeting reports and important interactions with the candidate.
                </p>
              </div>
              ...
              <Button
                size="sm"
                variant="outline"
                onClick={openProgramActivityModal}
              >
                <Plus className="size-4" />
            ...
            <CardContent className="px-0">
              {programActivities.length > 0 ? (
                <div className="overflow-x-auto">
                  <table className="w-full min-w-[950px] text-sm">
                    ...
                    <tbody>
                      {paginatedProgramActivities.map((activity) => (
                        <tr
                          key={activity.id}
                          className="border-b border-border/70 last:border-0"
                        >
                          ...
                          <td className="px-6 py-4 text-center">
                            <button
                              type="button"
                              className="inline-flex size-8 items-center justify-center rounded-md text-destructive hover:bg-destructive/10"
                              onClick={() => deleteProgramActivity(activity)}
                              title="Delete program activity"
                              aria-label="Delete program activity"
                            >
                              <Trash2 className="size-4" />
                            </button>
                          </td>
                ...
                <div className="border-t px-6 py-10 text-center">
                  ...
                  <p className="mt-3 text-sm font-medium">
                    No program activities recorded yet
                  </p>
                  <p className="mt-1 text-xs text-muted-foreground">
                    Add a meeting report after speaking with the candidate.
                  </p>
                  <Button
                    className="mt-4"
                    variant="outline"
              ...
              {programActivities.length > 8 && (
                <div className="mt-4 flex items-center justify-between px-6">
                  <p className="text-sm text-muted-foreground">
                    Page 1 of {totalProgramActivityPages} · {programActivities.length} activities
                  </p>
                </div>
              )}
            </CardContent>

            {programActivityModalOpen && (
              <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
                <div className="w-full max-w-2xl rounded-xl border bg-background p-6 shadow-xl">
                  <div className="mb-5 flex items-center justify-between">
                    <div>
                      ...
                      <p className="mt-1 text-sm text-muted-foreground">
                        Record a meeting or interaction with the candidate.
                      </p>
                    </div>
                    ...
                    <button
                      type="button"
                      className="inline-flex size-8 items-center justify-center rounded-md hover:bg-muted"
                      onClick={() => setProgramActivityModalOpen(false)}
                      aria-label="Close"
                    >
                      <X className="size-4" />
                    </button>
                  ...
                  <div className="grid gap-4 md:grid-cols-2">
                    <div>
                      <label className="mb-2 block text-sm font-medium">Meeting Date *</label>
                      <input
                        type="date"
                        value={programActivityDate}
                        onChange={(event) => setProgramActivityDate(event.target.value)}
                    ...
                    <div>
                      ...
                      <select
                        ...
                        onChange={(event) => setProgramActivityType(event.target.value)}
                        className="w-full rounded-md border bg-background px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-ring"
                      >
                        <option>Candidate Meeting</option>
                        <option>Progress Review</option>
                        <option>Follow-up Call</option>
                        <option>Program Discussion</option>
                        <option>Other</option>
                      </select>
                    </div>

                    <div>
                      <label className="mb-2 block text-sm font-medium">Conducted By *</label>
                      <input
                        type="text"
                        value={programActivityConductedBy}
                        onChange={(event) => setProgramActivityConductedBy(event.target.value)}
                    ...
                    <div>
                      ...
                      <input
                        type="text"
                        value={programActivitySubject}
                        onChange={(event) => setProgramActivitySubject(event.target.value)}
                        placeholder="Example: Resume review and job search progress"
                        className="w-full rounded-md border bg-background px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-ring"
                      />
                    </div>

                    <div className="md:col-span-2">
                      <label className="mb-2 block text-sm font-medium">Meeting Report *</label>
                      <textarea
                        value={programActivityNotes}
                        onChange={(event) => setProgramActivityNotes(event.target.value)}
                        rows={5}
                        placeholder="Enter what was discussed, candidate updates, concerns, decisions, and important notes..."
                  ...
                  <div className="mt-6 flex justify-end gap-3">
                    <Button
                      type="button"
                      variant="outline"
                      disabled={savingProgramActivity}
                      onClick={() => setProgramActivityModalOpen(false)}
                    >
                      Cancel
                    </Button>
                    <Button
                      type="button"
                      disabled={
                        savingProgramActivity ||
                        !programActivityDate ||
                        !programActivityConductedBy.trim() ||
                        !programActivityNotes.trim()
                      }
