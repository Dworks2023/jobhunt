...
assignedSpecialist:
  ...
  async function handleDeleteCandidate() {
    ...
    try {
      ...
      /*
       * IMPORTANT:
       * Candidate delete must go to the Express backend
       * running on port 5000, not the Vite frontend on 5173.
       */
      const deleteUrl =
  ...
  function exportCandidatesToPDF() {
    if (rows.length === 0) {
      alert("There are no candidates to export.");
      return;
    }
    ...
    doc.setFontSize(16);
    doc.text("Candidates", 14, 15);

    doc.setFontSize(9);
    doc.text(
      `Total Candidates: ${rows.length} | Exported: ${new Date().toLocaleDateString()}`,
      14,
      21,
    );

    const tableRows = rows.map((candidate) => [
      candidate.id || "-",
      candidate.name || "-",
      candidate.email || "-",
      candidate.domain || "-",
      ...
      String(candidate.daysRemaining ?? "-"),
      `${candidate.creditsRemaining ?? 0} / ${candidate.creditsTotal ?? 0}`,
      candidate.status || "Active",
    ]);

    // Draw the complete table directly with jsPDF.
    // This avoids requiring the separate jspdf-autotable package.
    const headers = [
      "ID",
      "Candidate",
      "Email",
    ...
    const columnWidths = [
      24, 28, 43, 27, 22, 18, 23, 23, 18, 25, 25,
    ];
  ...
  <div className="flex items-center justify-end gap-2">
    ...
    <div className="relative">
      ...
      {openActionMenu === candidate.id && (
        ...
        {/* PAGINATION */}
        {!loadingCandidates && rows.length > 0 && (
